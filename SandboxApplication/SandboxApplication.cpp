...
void SandboxApplication_createCubeEntity(SandboxApplicationCubeCreationInfo* p_sandboxCubeCreationInfo, _ECS::Entity** out_entity, _ECS::TransformComponent** out_entitytransform)
{
...

	{
		...
		_ECS::TransformComponent_init(l_component.Component, &l_transformInitInfo);
		*out_entitytransform = l_component.getChild();

		auto l_addComponentMessage = _ECS::ECSEventMessage_AddComponent_alloc(out_entity, &l_component.Component);
	...
	if (p_sandboxCubeCreationInfo->MeshRendererInitInfo)
	{
		...
		p_sandboxCubeCreationInfo->MeshRendererInitInfo->AssociatedComponent = l_component.Component;
		_ECS::MeshRenderer_init(l_component.getChild(), &App->Render.RenderInterface, p_sandboxCubeCreationInfo->MeshRendererInitInfo);
...
void SanboxApplication_createCubeCross(CubeCrossCreationInfo* p_cubeCrossCreationInfo)
{
	...
	if (p_cubeCrossCreationInfo->RotationAxis)
	{
		...
		auto l_transformRotate = l_component.getChild();
		l_transformRotate->Speed = .5f;
		l_transformRotate->Axis = *p_cubeCrossCreationInfo->RotationAxis;
		_ECS::Entity_addComponentDeferred(l_parentEntity, l_component.Component, &App->ECS);
	}
};
	...
	if (!HasAlreadyUpdated)
	{
		// Camera
		{
...

			{
				...
				auto l_camera = l_component.getChild();
				_ECS::Camera_init(l_camera, l_component.Component, &App->Render.RenderInterface);

				auto l_addComponentMessage = _ECS::ECSEventMessage_AddComponent_alloc(&l_cameraEntity, &l_component.Component);
				_ECS::ECSEventQueue_pushMessage(&App->ECS.EventQueue, &l_addComponentMessage);
			}
		...
		// Ray
		{
...

			{
				...
				auto l_component = _ECS::Component_allocV2<_ECS::TransformComponent>();
				l_rayEntityTransform = l_component.getChild();
				...
				auto l_addComponentMessage = _ECS::ECSEventMessage_AddComponent_alloc(&l_rayEntity, &l_component.Component);
				_ECS::ECSEventQueue_pushMessage(&App->ECS.EventQueue, &l_addComponentMessage);
			}
			...
			*/
			...
			//Ray Begin
			{
				...
				{
					auto l_component = _ECS::Component_allocV2<_ECS::TransformComponent>();
					l_rayBeginEntityTransform = l_component.getChild();
					...
					auto l_addComponentMessage = _ECS::ECSEventMessage_AddComponent_alloc(&l_rayBeginEntity, &l_component.Component);
					_ECS::ECSEventQueue_pushMessage(&App->ECS.EventQueue, &l_addComponentMessage);
			...
			//Ray End
			{
				...
				{
					auto l_component = _ECS::Component_allocV2<_ECS::TransformComponent>();
					l_rayEndEntityTransform = l_component.getChild();
					...
					auto l_addComponentMessage = _ECS::ECSEventMessage_AddComponent_alloc(&l_rayEndEntity, &l_component.Component);
					_ECS::ECSEventQueue_pushMessage(&App->ECS.EventQueue, &l_addComponentMessage);
		...
		// Scene root
		{
...

			{
				...
				auto l_component = _ECS::Component_allocV2<_ECS::TransformComponent>();
				l_sceneModelsRootTransform = l_component.getChild();
				...
				auto l_addComponentMessage = _ECS::ECSEventMessage_AddComponent_alloc(&l_sceneModelsRootEntity, &l_component.Component);
				_ECS::ECSEventQueue_pushMessage(&App->ECS.EventQueue, &l_addComponentMessage);
