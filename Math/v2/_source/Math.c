...
void Mat_Mul_M4F_M4F(const Matrix4f_PTR p_left, const Matrix4f_PTR p_right, Matrix4f_PTR p_out)
{
	...
	for (int i = 0; i < 16; i += 4) {
		...
		a_line = _mm_load_ps(p_left->Points);         // a_line = vec4(column(a, 0))
		b_line = _mm_set_ps1(p_right->Points[i]);      // b_line = vec4(b[i][0])
		r_line = _mm_mul_ps(a_line, b_line); // r_line = a_line * b_line
		for (int j = 1; j < 4; j++) {
			a_line = _mm_load_ps(&p_left->Points[j * 4]); // a_line = vec4(column(a, j))
			b_line = _mm_set_ps1(p_right->Points[i + j]);  // b_line = vec4(b[i][j])
