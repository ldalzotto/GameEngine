...
namespace _CoreV3
{
	...
	template <typename T>
	__forceinline size_t Hash(T* p_value)
	{
		return std::hash<T>()(*p_value);
	};

	template < typename T, template < class T > class DATA_STRUCTURE >
	__forceinline size_t Hash(DATA_STRUCTURE<T>* p_value)
	{
		return Hash(p_value->Memory);
	}

	template <typename HASHED_TYPE>
	__forceinline size_t HashCombine(size_t p_seed, HASHED_TYPE* p_right)
	{
	...
	__forceinline size_t HashCombine(size_t p_seed, HASHED_TYPE* p_hashedType, HASHED_TYPES... p_right)
	{
