...
Core_ReturnCodes Core_GenericArray_erase(Core_GenericArray* p_genericArray, size_t p_index)
{
	if (p_index >= p_genericArray->Size)
	{
		ERR_PUSH(CR_OUT_OF_BOUND, "Core_GenericArray_erase : Erase out of range.");
	}

	// If we are not erasing the last element, then we move memory. Else, we have nothing to do.
	if (p_index + 1 != p_genericArray->Size)
	{
		void* p_targetMemory = (char*)p_genericArray->Memory + Core_GenericArray_getElementOffset(p_genericArray, p_index);
...
void Core_GenericArray_eraseCompare(Core_GenericArray* p_genericArray, Core_Comparator* p_comparator)
{
	...
	for (size_t i = 0; i < p_genericArray->Size; i++)
	{
		...
		{
			ERR_THROW(Core_GenericArray_erase(p_genericArray, i));
			break;
		}
...
{
	Core_GenericArray l_copiedArray;
	memcpy(&l_copiedArray, p_genericArray, sizeof(Core_GenericArray));
	l_copiedArray.Memory = malloc(Core_GenericArray_getTotalSize(p_genericArray));
	memcpy(l_copiedArray.Memory, p_genericArray->Memory, Core_GenericArray_getTotalSize(p_genericArray));
