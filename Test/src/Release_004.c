...
void ComplexShape(int argc, char* argv[])
{
	...
	Matrix4f_PTR l_modelMatrix_PTR = &l_modelMatrix;
	BoxF l_meshBoundingBox = { 0 };
	RenderedObject l_renderableObject;
	RenderedObject_PTR l_renderableObjectPTR = &l_renderableObject;

	Arr_PushBackRealloc_RenderedObjectHandle(&l_renderedObjects, &l_renderableObjectPTR);
	{
		...
		RendereableObject_PushToRenderEngine(&renderV2.GlobalBuffer.RenderedObjectBuffers, &l_renderableObject);
