...
{
	void init(ImageViews* p_imageViews, const ImageViewsDependencies& p_imageViewsDependencies, 
		SwapChainInfo* p_swapChainInfo,
		std::vector<VkImage>* p_swapChainImages)
	{
		p_imageViews->ImageViewsDependencies = p_imageViewsDependencies;

		p_imageViews->ImageViews.resize(p_swapChainImages->size());
		...
		for (size_t i = 0; i < p_imageViews->ImageViews.size(); i++)
		{
			...
			l_imageCreate.image = p_swapChainImages->at(i);
			l_imageCreate.viewType = VK_IMAGE_VIEW_TYPE_2D;
			l_imageCreate.format = p_swapChainInfo->SurfaceFormat.format;
