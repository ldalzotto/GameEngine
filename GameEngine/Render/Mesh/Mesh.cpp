...
namespace _GameEngine::_Render::_Mesh
{
	void Mesh_alloc(Mesh* p_mesh, MeshAllocInfo* p_meshAllocInfo)
	{
		...
		_Memory::VulkanBuffer_pushToGPU(&p_mesh->VertexBuffer, p_meshAllocInfo->Device, p_mesh->Vertices.data(), static_cast<size_t>(p_mesh->VertexBuffer.BufferCreateInfo.size));
		...
		_Memory::VulkanBuffer_pushToGPU(&p_mesh->IndicesBuffer, p_meshAllocInfo->Device, p_mesh->Indices.data(), static_cast<size_t>(p_mesh->IndicesBuffer.BufferCreateInfo.size));
	};
