...
namespace _GameEngine::_Render
{
	void DefaultMaterialDrawStep_buildCommandBuffer(SwapChain* p_swapChain, DefaultMaterialDrawStep* p_meshDrawStep, VkCommandBuffer p_commandBuffer, size_t l_imageIndex)
	{
		...
		for (MaterialInstance* l_materialInstance : p_meshDrawStep->MaterialInstances)
		{
			...
			vkCmdBindDescriptorSets(p_commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *l_materialInstance->PipelineLayout, 1, 1, &l_materialInstance->MaterialDescriptorSet, 0, nullptr);
	...
	{
		p_meshDrawStep->MaterialInstances.clear();
	};
}